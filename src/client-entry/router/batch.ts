...
export default function (list: Array<TTransactionData>, state: IState<IUser>): Promise<Array<TTransactionWithProofs<TLong> & IWithId>> {
    return new Promise((resolve, reject) => {
        renderPage(batchPage({
            ...
            list,
            onConfirm: () => {
                resolve(list.map(item => signTx(item.extended as any, state.user.seed)) as any); // TODO Fix types
            },
            onCancel: () => {
