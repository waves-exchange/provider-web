...
export function addUser(password: string, networkByte: number, rounds?: number): IUser {
    console.log(networkByte, String.fromCharCode(networkByte));
    const seed = 'merry help cycle scrub adult element initial old devote moon waste inside steel version post'; //libs.crypto.randomSeed(15);
    ...
    const id = libs.crypto.base58Encode(
        libs.crypto.blake2b(
            libs.crypto.base58Decode(networkByte + publicKey)
        )
    );

    const users = JSON.parse(localStorage.getItem('multiAccountData') || '{}');
    const json = JSON.stringify({ ...users, [id]: { seed, publicKey, networkByte: networkByte, userType } });
    const multiAccountHash = libs.crypto.base58Encode(libs.crypto.blake2b(libs.crypto.stringToBytes(json)));
    const multiAccountData = seedUtils.encryptSeed(json, password, rounds || 5000);
